...
- If not, In openshift for example whether the api server already exposes or can be configured to expose the OIDC discovery endpoint
...
| External         | No, Openshift              | Yes                        | [JWT pre-reqs + setup](#jwt-auth-pre-requisites)                        |
