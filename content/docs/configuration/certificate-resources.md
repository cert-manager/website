...
description: Learn how to create, renew and manage SSL / TLS certificates using the cert-manager Certificate resource.
---

The `Certificate` resource represents a desired X.509 certificate which will be signed and renewed before it expires.
...
A `Certificate` is a namespaced resource which references an `Issuer` or `ClusterIssuer`,
and it is the referenced `Issuer` or `ClusterIssuer` which dictates how cert-manager will sign your SSL / TLS certificate.

> ℹ️ In order to issue any certificates, you'll first need to configure an `Issuer` or `ClusterIssuer`.
>
...
Here is an example of a `Certificate` resource.
It contains an exhaustive list of all the options a `Certificate` resource may have,
however you will probably only need a subset of these fields.
...
This Certificate uses the DNS names: `example.com` and `www.example.com`.
It has a Subject Alternative Name for the URI: `spiffe://cluster.local/ns/sandbox/sa/example`.
And it is valid for 90 days and renews 15 days before expiry.

> ℹ️ The `dnsNames` field specifies a list of [`Subject Alternative Names`](https://en.wikipedia.org/wiki/Subject_Alternative_Name) to be associated with the certificate.

> ℹ️ The referenced `Issuer` must exist in the same namespace as the `Certificate`.
...
> ℹ️ If you want to create an `Issuer` that can be referenced by
...
>
> ℹ️ The `renewBefore` and `duration` fields must be specified using a [Go
...
>
> ⚠️ Take care when setting the `renewBefore` field to be very close to the
...
## Configuring the Certificate Renewal Schedule
...
## The Certificate Chain in `tls.crt`
