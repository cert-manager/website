...
func TestUpdateChangelogSection(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			// A legacy file may have a bare ## `vX.Y.Z` heading but no markers.
			// The tool replaces the entire section with the generated changelog wrapped in markers.
			// On the next run the marker will be found and generation will be skipped.
			name: "legacy file with bare version heading",
			in: dedent(`
			  # Release Notes
			  Some intro.
		...
		{
			// When the legacy heading is for a different version (e.g. an older patch),
			// the section is left untouched and no markers are inserted.
			name: "legacy file with non-matching version heading",
			in: dedent(`
			  # Release Notes
...
func TestInsertMissingMarkers(t *testing.T) {
	type testCase struct {
		name string
		in   string
		out  string
	...
	testCases := []testCase{
		{
			name: "no-op when markers already present",
			in: dedent(`
			  # Release Notes
			  {/* BEGIN changelog v1.2.3 */}
			  ## ` + "`v1.2.3`" + `
		...
		{
			name: "no-op when no recognisable structure",
			in: dedent(`
			  # Release Notes
			  Some initial content without any known sections.
			  `),
			out: dedent(`
			  # Release Notes
			  Some initial content without any known sections.
		...
		{
			// Changelog markers are NOT inserted by insertMissingMarkers.
			// A bare version heading is left in place; updateChangelogSection
			// replaces it with generated content on the first tool run.
			name: "does not insert changelog markers around bare version heading",
			in: dedent(`
			  # Release Notes
			  Some intro.
		...
		{
			name: "inserts community-section markers using template anchors",
			in: dedent(`
			  ## Community
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got := insertMissingMarkers([]byte(tc.in))
			assert.Equal(t, tc.out, string(got))
		})
	}
}
