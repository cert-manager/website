...
import remarkAdmonitions from 'lib/remark-plugins/admonitions'


export async function getArticles() {
  // First, get all files in content/announcements
  const files = await readdir(join(process.cwd(), 'content', 'announcements'));

  // Filter out any files which don't end in .md (e.g. vim .swp files)
  const mdFiles = files.filter(file => file.endsWith('.md'));

  // TODO: if this promise fails the error message is very confusing, but that only affects dev
...
export async function pageProps({params}) {
  ...
  const mdxSource = await serialize(
    ...
    }),
    {
      ...
      scope: { data },
      // Allow JS expressions (e.g. JSX-style inline styles) in MDX content.
      // blockJS is a top-level serialize option in next-mdx-remote v6.
      blockJS: false,
      mdxOptions: {
        mdExtensions: [],
