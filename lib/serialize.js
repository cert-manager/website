...
export async function pageProps({ params }) {
  ...
  const mdxSource = await serialize(
    ...
    }),
    {
      ...
      scope: { data },
      mdxOptions: {
        // next-mdx-remote v6 blocks JS expressions by default. Setting
        // blockJS: false restores support for JSX/JS expressions (e.g.
        // style={{...}}) used in existing MDX content. blockDangerousJS
        // remains true (the default) to keep security protections in place.
        blockJS: false,
        mdExtensions: [],
